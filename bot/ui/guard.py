...
def channel_problem(channel: Chat) -> str | None:
    """Почему подписку на канал не получится проверить или предложить."""
    ...
    if not channel.username and not channel.invite_link and channel.can_invite is False:
        return "закрытый канал, а у бота нет права «Пригласительные ссылки» — в подсказке не будет кнопки"
    return None
...
async def sub_settings_view(app: App, note: str | None = None) -> Screen:
    ...
    lines = [
        ...
        "сразу удаляет и на минуту показывает подсказку с кнопками «📢 Подписаться» и «✅ Я подписался». "
