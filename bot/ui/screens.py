...
async def chat_view(app: App, chat_id: int) -> Screen | None:
    ...
    if chat.status == "pending":
        ...
        if chat.type != "channel":  # для /ban и /mute
            rights += f" · {'✅' if chat.can_restrict else '❌'} блокировка"
        lines.append(rights)
        if not chat.can_post:
...
async def settings_view(app: App, note: str | None = None) -> Screen:
    ...
    lines = [
        ...
        f"🕶 Скрытые админы во всех группах: {len(app.settings.trusted)}",
        "",
    ...
    rows = [
        ...
        [btn("🚪 Автоприём заявок", Guard(a="joins")), btn("🕶 Скрытые админы", Guard(a="trust"))],
        [
...
def help_view() -> Screen:
    text = (
        ...
        "рекламу от имени каналов, стоп-слова, слишком длинные сообщения (лимит — в «🛡 Антиспам»). "
        "«⚙️ Настройки → 🔒 Обязательная подписка» — писать смогут только "
        "подписчики выбранных каналов: остальным бот показывает личные ссылки на каналы и кнопку "
        "«✅ Проверить подписку». Правила чата — «🛡 Защита» на экране чата. Скрытые админы (кнопка внизу "
        "экрана или «⚙️ Настройки → 🕶 Скрытые админы») пишут без проверок, как админы чата. Боту нужно право "
        ...
        "<b>6. Команды в группах</b> — ответом на сообщение нарушителя: /del — удалить, /mute и /ban — замутить "
        "и забанить (навсегда или на срок: <code>/mute 1h</code>, <code>/ban 1day</code>), /delmute и /delban — "
        "удалить и наказать, /unmute и /unban — снять. Команда сразу исчезает, в чате не остаётся следов. "
        "Могут админы чата с нужным правом, админы бота и скрытые админы; боту нужно право "
        "«Блокировка пользователей».\n\n"
        "<b>7. Черновики.</b> «💾 В черновики» сохраняет посты, расписание и опции. «📤 Применить к чатам» "
